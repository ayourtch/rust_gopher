...
fn main() -> std::io::Result<()> {
    let matches = App::new("rust_gopher")
        ...
        .arg(
            Arg::with_name("send-port")
                .short("s")
                .long("send-port")
                .takes_value(true)
                .help("Port number to send to clients"),
        )
        .arg(
            Arg::with_name("dir")
                .short("d")
    ...
    let send_port: i16 = matches
        .value_of("send-port")
        .unwrap_or("70")
        .parse()
        .unwrap();
    let dir = matches.value_of("dir").unwrap_or("root").to_string();
...
fn handle_v1(mut stream: TcpStream, request: &str, config: &Config) -> std::io::Result<()> {
    ...
    if path.exists() {
            ...
            if map.exists() {
                ...
                for l in lines {
                    if let Err(e) = stream.write(
                        format!(
                            "{}{}\t{}\t{}\t{}\r\n",
                            l.row_type, l.label, l.path, l.server, l.port
                        )
                        .as_bytes(),
                    ) {
                        println!("could not send gophermap: {:?}", &e);
                    }
                }
            }
        } else {
        ...
        if let Err(e) = stream.write(&error.as_bytes()) {
